...
.pd-show-mentor-photo{width:70px;height:70px;border-radius:10px;overflow:hidden;flex-shrink:0;position:relative;background:#1a1a1a;padding:2px}
.pd-show-mentor-photo::before{content:'';position:absolute;inset:-1px;border-radius:11px;padding:1.5px;background:linear-gradient(135deg,#fd1c00,#EEA727,#fd1c00);background-size:200% 200%;animation:mentorBorder 3s ease infinite;-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask-composite:exclude;pointer-events:none}
.pd-show-mentor-photo>img,.pd-show-mentor-photo>.pd-show-mentor-fallback{border-radius:8px;position:relative;z-index:1}
@keyframes mentorBorder{0%{background-position:0% 50%}50%{background-position:100% 50%}100%{background-position:0% 50%}}
...
.pd-member-name-big{font-family:'Astro','Orbitron',sans-serif;font-size:.62rem;font-weight:800;color:#fff;text-shadow:0 2px 12px rgba(0,0,0,.95),0 0 8px rgba(253,28,0,.3);letter-spacing:2px;text-transform:uppercase;writing-mode:vertical-lr;transform:rotate(180deg);line-height:1}
.pd-member-name-sub{display:none}
.pd-member-roll-rt{position:absolute;top:10px;right:8px;z-index:4;font-family:'DM Sans',sans-serif;font-size:.5rem;color:rgba(255,255,255,.75);font-weight:700;writing-mode:vertical-lr;letter-spacing:1.5px;text-shadow:0 2px 10px rgba(0,0,0,.95);padding:6px 3px;background:rgba(0,0,0,.25);border-radius:4px;backdrop-filter:blur(4px)}
