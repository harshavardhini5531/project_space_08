...
export async function sendMail(mailOptions) {
  try {
    ...
    if (transporter2 && (err.responseCode === 550 || err.code === 'EENVELOPE')) {
      mailOptions.from = mailOptions.from.replace(process.env.GMAIL_USER, process.env.GMAIL_USER2)
      return await transporter2.sendMail(mailOptions)
    }
    throw err
  }
}
