...
// ━━━ Gmail fallback chain (for all emails) ━━━
async function sendViaGmail(mailOptions) {
  ...
  if (chain.length === 0) {
    throw new Error('All Gmail senders exhausted (daily limits hit).')
  }
  ...
  throw lastError || new Error('All Gmail senders failed')
}

// ━━━ sendMail — Power Automate first, then Gmail ━━━
export async function sendMail(mailOptions) {
  // Try Power Automate first (works for all emails)
  if (process.env.POWER_AUTOMATE_URL) {
    try {
      ...
      console.log(`✅ Sent via PowerAutomate (Outlook) to ${mailOptions.to}`)
      return { via: 'PowerAutomate' }
    } catch (err) {
...
// ━━━ OTP Email — Power Automate primary ━━━
