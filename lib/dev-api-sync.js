...
const REQUEST_TIMEOUT_MS = 10_000; // 10s — give dev's API time to respond
const MAX_RETRIES = 3;
...
function buildPayload(submission) {
  ...
  return {
    ...
    projectType: submission.project_type || 'default',
...
export async function syncSubmissionToDevApi(submission) {
  ...
  // ───── 3. Skip if max retries exhausted (caller can override by passing skipMaxCheck) ─────
  ...
  if (result.ok && result.body?.success && result.body?.data?._id) {
    ...
    if (updateErr) {
      ...
      // Sync to dev API succeeded but our DB update failed
      // Return ok=true but with warning
      return {
        ok: true,
        dev_api_id: devApiId,
  ...
  errorMsg = errorMsg.slice(0, 500); // cap length
...
// ─────────────────────────────────────────────────────────────────
// Helper: get submissions that need retry
// (used by retry cron)
// ─────────────────────────────────────────────────────────────────
/**
 * Returns submissions that need to be re-synced to dev API.
 * Filters: not yet synced AND retries < MAX AND last attempt > 5 min ago
 *
...
export async function getPendingDevApiSync(limit = 50) {
  ...
  const { data, error } = await supabase
    ...
    .order('dev_api_retry_count', { ascending: true }) // try fresh ones first
